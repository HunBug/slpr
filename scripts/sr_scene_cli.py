...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory (defaults to outputs/<stamp>/<scene_stem>/)",
    )
    p.add_argument(
...
def assemble_gif(frame_paths: list[Path], out_path: Path, fps: int) -> None:
    ...
    # Stubs in imageio can be strict; ignore type noise at call-site.
    iio.mimsave(  # type: ignore
        str(out_path), frames, duration=duration, loop=0
    )
